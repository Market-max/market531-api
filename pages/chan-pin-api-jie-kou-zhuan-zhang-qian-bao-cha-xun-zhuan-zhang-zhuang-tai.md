...
该API接口由**POLY平台**提供,由**接入方**进行调用 **`POST`** `{`**`POLY_APIURL`**`}/api/cash/v1/transfer/status?trace_id=`**`your`**`_trace_id` `{`**`POLY_APIURL`**`}`为POLY接口域名,可通过后台获得
...
```json
{
    ...
    "tx_id": "50f15a92-ee8d-11ee-9a61-00ff3fae08f7",
...
| state    | Yes | integer | 转账状态0:失败 1:成功 2:Pending                               |
